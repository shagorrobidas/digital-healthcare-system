...
class HealthcareSystem:
    ...
    def create_main_menu(self):
        ...
        ttk.Button(frame, text="Logout", command=self.create_login_screen, width=25).pack(pady=20)
    
    def check_appointment_reminders(self):
        cursor = self.conn.cursor()
        tomorrow = (datetime.now() + timedelta(days=1)).strftime("%Y-%m-%d")
        if self.current_role == "doctor":
            cursor.execute("SELECT patient_id, date, time FROM appointments WHERE date = ? AND status = 'Scheduled' AND doctor_id = ?",
                          (tomorrow, self.current_user_id))
        else:
            cursor.execute("SELECT patient_id, date, time, doctor_id FROM appointments WHERE date = ? AND status = 'Scheduled'",
                          (tomorrow,))
        appointments = cursor.fetchall()
        if appointments:
            reminder = "\n".join([f"Reminder: Patient ID {row[0]} at {row[2]} on {row[1]}" 
    ...
    def patient_management(self):
        self.check_session_timeout()
        self.clear_window()
        tk.Label(self.root, text="Patient Management", font=("Arial", 14, "bold")).pack(pady=10)
        ...
        ttk.Button(search_frame, text="Search", command=lambda: self.search_patients(search_entry.get())).pack(side=tk.LEFT)
        ...
        tk.Label(frame, text="DOB (YYYY-MM-DD):", font=("Arial", 10)).grid(row=1, column=0, padx=5, pady=5)
        dob_entry = tk.Entry(frame)
        dob_entry.grid(row=1, column=1, padx=5, pady=5)
        
        tk.Label(frame, text="Contact:", font=("Arial", 10)).grid(row=2, column=0, padx=5, pady=5)
        contact_entry = tk.Entry(frame)
        contact_entry.grid(row=2, column=1, padx=5, pady=5)
        
        tk.Label(frame, text="Email:", font=("Arial", 10)).grid(row=3, column=0, padx=5, pady=5)
        email_entry = tk.Entry(frame)
        email_entry.grid(row=3, column=1, padx=5, pady=5)
        
        tk.Label(frame, text="Address:", font=("Arial", 10)).grid(row=4, column=0, padx=5, pady=5)
        address_entry = tk.Entry(frame)
        address_entry.grid(row=4, column=1, padx=5, pady=5)
        
        tk.Label(frame, text="Gender:", font=("Arial", 10)).grid(row=5, column=0, padx=5, pady=5)
        gender_var = tk.StringVar(value="Male")
        tk.Radiobutton(frame, text="Male", variable=gender_var, value="Male").grid(row=5, column=1, sticky="w")
        tk.Radiobutton(frame, text="Female", variable=gender_var, value="Female").grid(row=5, column=1, sticky="e")
        
        tk.Label(frame, text="Blood Group:", font=("Arial", 10)).grid(row=6, column=0, padx=5, pady=5)
        blood_group_var = tk.StringVar()
        blood_groups = ["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-"]
        ttk.Combobox(frame, textvariable=blood_group_var, values=blood_groups).grid(row=6, column=1, padx=5, pady=5)
        
        tk.Label(frame, text="Emergency Contact:", font=("Arial", 10)).grid(row=7, column=0, padx=5, pady=5)
        emergency_contact_entry = tk.Entry(frame)
        emergency_contact_entry.grid(row=7, column=1, padx=5, pady=5)
        
        tk.Label(frame, text="Medical History:", font=("Arial", 10)).grid(row=8, column=0, padx=5, pady=5)
        ...
        ttk.Button(frame, text="Add Patient", command=lambda: self.add_patient(
            name_entry.get(), dob_entry.get(), contact_entry.get(), email_entry.get(),
        ...
        self.patient_tree = ttk.Treeview(self.patient_tree_frame, 
                                        columns=("ID", "Name", "DOB", "Contact", "Email", "Gender", "Blood Group"), 
                                        show="headings")
    ...
    def show_patient_details(self, event):
        ...
        tk.Label(details_window, text="Patient Details", font=("Arial", 12, "bold")).pack(pady=10)
        
        fields = ["ID", "Name", "DOB", "Contact", "Email", "Address", "Gender", "Blood Group", "Emergency Contact", "Medical History"]
        for i, (field, value) in enumerate(zip(fields, patient)):
            tk.Label(details_window, text=f"{field}: {value}").pack(pady=5)
        
        if self.current_role == "doctor":
            ttk.Button(details_window, text="View Appointment History", 
                      command=lambda: self.view_patient_appointment_history(patient_id)).pack(pady=5)
            ttk.Button(details_window, text="View Prescription History", 
                      command=lambda: self.view_patient_prescription_history(patient_id)).pack(pady=5)
